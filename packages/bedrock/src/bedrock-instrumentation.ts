...
import { GEN_AI_EVENTS } from '@reaatech/otel-genai-semconv-core';
import type { PricingInfo } from '@reaatech/otel-genai-semconv-core';
...
export class BedrockInstrumentation {
  ...
  private startSpan(input: { modelId: string; body: string }, modelId: string): Span {
    ...
    const modelFamily = modelId.split('.')[0] ?? modelId;
