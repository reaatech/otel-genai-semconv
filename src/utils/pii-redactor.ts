/**
 * PII redaction utility for sensitive data handling
 */
...
export interface PIIRedactionOptions {
  ...
  /** Whether to hash instead of redact (for debugging) */
...
/**
 * PII redactor for sensitive data handling
 */
export class PIIRedactor {
  ...
  redact(text: string): string {
    ...
    for (const pattern of this.patterns) {
      ...
      if (this.options.hashInsteadOfRedact) {
        // Hash matched values instead of replacing
  ...
  /**
   * Whether message content should be redacted
   */
  ...
  /**
   * Include a previously excluded pattern
   */
  ...
  /**
   * Hash a value for debugging using FNV-1a algorithm
   */
